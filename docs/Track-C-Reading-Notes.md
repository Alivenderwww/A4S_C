...
> **⚠️ 本文档记录的是赛题规则与背景知识，不代表当前实现状态。**
> 来源：`public/Track-C/` 下的官方文档 + Wikipedia（PTX / ONNX / CUDA）补充。
> 本文档仅做重点摘要，便于快速把握赛题框架与难点。
>
> **C1 编译器当前实现状态**请以 [`C1-完成度审计.md`](./C1-完成度审计.md) 为准。
> 本文若与审计文档发生直接冲突，以审计文档为准。历史分析和背景知识仍然有效。
...
- AEC C1 流水线为 **列表调度(pre-RA) → 线性扫描分配**：调度先于寄存器分配，用于降低寄存器压力。
...
### C1 难度最高
- 需要完整的编译器栈（IR → Opt → 列表调度(pre-RA) → 线性扫描 → 降级）
- 当前优先级与 TODO 详见 [`C1-完成度审计.md`](./C1-完成度审计.md)。
